namespace :all do
  task :start do
    ...
    `RAILS_ENV=#{Rails.env} bin/delayed_job --queues=actions,browsers,selectors,sibling_relationships,object_types,backup,prune -i=1 start`
  ...
  task :stop do
    ...
    `RAILS_ENV=#{Rails.env} bin/delayed_job --queues=actions,browsers,selectors,sibling_relationships,object_types,backup,prune -i=1 stop`
